...
func (c *Client) GroupAddServer(group, host, port, user string, options *GroupAddServerOptions) (*GroupServer, error) {
	args := []string{"--group", group, "--host", host, "--port", fmt.Sprintf("%q", port), "--user", fmt.Sprintf("%q", user)}
	if options != nil {
...
func (c *Client) GroupDelServer(group, host, port, user string, proxyOpts *ProxyOptions) error {
	args := []string{"--group", group, "--host", host, "--port", port, "--user", user}
	if proxyOpts != nil {
